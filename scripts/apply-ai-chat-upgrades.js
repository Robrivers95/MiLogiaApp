...
patch('functions/src/index.ts', source => {
  ...
  if (!source.includes('const DAILY_AI_LIMIT = 10;')) {
    source = source.replace(
  ...
  if (!source.includes('Ya alcanzaste el límite de 10 consultas')) {
    source = source.replace(
      "      const apiKey = process.env.GEMINI_API_KEY;\n      if (!apiKey) throw new Error('GEMINI_API_KEY no está configurada');",
      "      const rateLimit = await consumeDailyAIQuery(decoded.uid);\n      if (!rateLimit.allowed) {\n        res.status(429).json({ error: 'Ya alcanzaste el límite de 10 consultas de IA por hoy. Podrás volver a consultar mañana.', rateLimit });\n        return;\n      }\n\n      const apiKey = process.env.GEMINI_API_KEY;\n      if (!apiKey) throw new Error('GEMINI_API_KEY no está configurada');"
    );
  ...
  source = source.replace(
    "        `If the petition asks how much a person owes or what pending items they have, use member-pending and extract memberName.\\n` +",
    "        `If the petition asks how much a person owes or what pending items they have, use member-pending and extract memberName.\\n` +"
  );
