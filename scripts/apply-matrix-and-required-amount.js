...
patch('components/Payments.tsx', source => {
  if (!source.includes("Ingresa el monto transferido; puede ser un abono parcial.")) {
    ...
    source = source.replace('Monto transferido (opcional)', 'Monto transferido *');
    source = source.replace(
...
patch('components/Admin.tsx', source => {
  if (!source.includes('Desglose según filtro activo')) {
    const startMarker = '            <div className="bg-logia-900 rounded p-3 text-sm space-y-1">\n              <p className="text-gray-400">Miembro:';
    ...
    if (start !== -1 && next !== -1) {
      const newSummary = [
        '            <div className="bg-logia-900 rounded p-3 text-sm space-y-2">',
        ...
        '                const visibleFees = (matrixModalPayment.extraFees || []).filter(fee => matrixFilter === \'all\' || fee.description === matrixExtraDesc);',
        '                const legacyMatches = !matrixModalPayment.extraFees?.length && Number(matrixModalPayment.extraAmount || 0) > 0 && (matrixFilter === \'all\' || (matrixModalPayment.extraDescription || \'Cuota Extra\') === matrixExtraDesc);',
      ...
      source = source.slice(0, start) + newSummary + source.slice(next);
      source = source.replace(
        'Monto pagado (deja vacío para marcar como pagado completo)',
        'Monto recibido para la cuota mensual'
      );
    }
  }
  return source;
