...
const NotificationBell: React.FC<Props> = ({ user, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(notificationsQuery, snap => {
      ...
      if (list.length > prevCountRef.current && prevCountRef.current !== -1) {
        const newest = list[0];
        if (newest) notificationService.showBrowserNotification(newest.title, newest.body, newest.type);
      }
  ...
  return (
    <>
        ...
        {open && (
          <div className="fixed right-2 top-[65px] w-80 max-w-[calc(100vw-1rem)] max-h-[70vh] bg-logia-800 border border-logia-700 rounded-xl shadow-2xl z-[200] flex flex-col overflow-hidden">
            <div className="flex justify-between items-center px-4 py-3 border-b border-logia-700 bg-logia-900">
              <div>
                <h3 className="font-bold text-white text-sm">Buzón y notificaciones</h3>
                <p className="text-[10px] text-gray-500">Avisos, tareas, comprobantes e imágenes</p>
              </div>
              {unreadCount > 0 && (
                <button onClick={handleMarkAllRead} className="text-xs text-indigo-400 hover:text-indigo-200 underline">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {imageUrl && (
                        <div className="mt-2 rounded-lg overflow-hidden border border-white/10 bg-black/20">
                          <img src={imageUrl} alt="Imagen adjunta" className="w-full h-24 object-cover" loading="lazy" />
                          <p className="text-[10px] text-center py-1 text-amber-300">Toca para ver la imagen completa</p>
                        </div>
                      )}
                      <p className="text-gray-600 text-[10px] mt-1">
                        {new Date(notif.createdAt).toLocaleString('es-MX', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                      </p>
                    </div>
                    <button
                      className="text-gray-500 hover:text-green-400 text-lg flex-shrink-0 self-start"
                      title="Marcar como leída"
                      onClick={event => { event.stopPropagation(); void notificationService.markRead(user.uid, notif.id); }}
                    >✓</button>
                  </div>
                );
              })}
      ...
      {previewImage && (
        <div className="fixed inset-0 z-[300] bg-black/85 p-3 flex items-center justify-center" onClick={() => setPreviewImage(null)}>
          <div className="max-w-6xl w-full max-h-[94vh] overflow-auto rounded-2xl bg-logia-900 border border-amber-500/40 p-3" onClick={event => event.stopPropagation()}>
            <div className="flex justify-between items-center mb-2">
              <p className="text-white font-bold">Imagen recibida en el buzón</p>
              <button className="text-white text-xl px-2" onClick={() => setPreviewImage(null)} aria-label="Cerrar">✕</button>
            </div>
            <img src={previewImage} alt="Matriz de pagos" className="max-w-none min-w-full h-auto rounded-lg bg-white" />
          </div>
        </div>
      )}
    </>
