...
const Admin: React.FC<Props> = ({ user }) => {
  ...
  const handleSaveEditedAttendance = async () => {
      ...
      try {
          ...
          // Borrar registros de quienes estaban presentes pero fueron desmarcados
          const prevPresentUids = attDetailList.filter(item => item.attended).map(item => item.uid);
          const toDelete = prevPresentUids.filter(uid => !editAttSelected.has(uid));
          await Promise.all(toDelete.map(uid => dataService.deleteAttendance(uid, editingAttDate!)));
