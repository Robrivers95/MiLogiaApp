    ...
    if (isRegistering) {
      (async () => {
        try {
          ...
          setGroups(gs);
          // NO seleccionar automáticamente - forzar selección manual
          setSelectedGroupId('');
        } catch (e) {
          console.error('Error cargando logias:', e);
          setError('No se pudieron cargar las logias. Intenta recargar la página.');
        } finally {
          setLoadingGroups(false);
        }
      })();
    } else {
      // Limpiar al volver a login
      setGroups([]);
      setSelectedGroupId('');
    }
  }, [isRegistering]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isRegistering) {
        ...
        if (groups.length === 0) throw new Error('No hay logias disponibles. Contacta al administrador.');
        if (!selectedGroupId) throw new Error('Debes seleccionar una Logia para continuar');
          ...
          {isRegistering && (
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">
                Logia <span className="text-red-400">*</span>
              </label>
              {loadingGroups ? (
                <div className="w-full bg-logia-900 border border-logia-700 rounded-lg p-3 text-gray-400">
                  Cargando logias...
                </div>
              ) : groups.length === 0 ? (
                <div className="w-full bg-red-900/20 border border-red-500/50 rounded-lg p-3 text-red-200 text-sm">
                  ⚠️ No se encontraron logias disponibles. Contacta al administrador.
                </div>
              ) : (
                <select
                  className="w-full bg-logia-900 border border-logia-700 rounded-lg p-3 text-white"
                  ...
                  required
                >
                  <option value="">-- Selecciona tu Logia --</option>
                  {groups.map(g => (
